...
FN_INTERNAL int fnusb_num_devices(fnusb_ctx *ctx)
{
	...
	for (i = 0; i < cnt; ++i)
	{
		...
		if (r < 0)
			continue;
		if (desc.idVendor == VID_PRIMESENSE && desc.idProduct == PID_XTION)
			nr++;
	}
...
FN_INTERNAL int fnusb_open_subdevices(freenect_device *dev, int index)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (desc.idVendor != VID_PRIMESENSE)
			continue;

		// Search for the camera
		if ((ctx->enabled_subdevices & FREENECT_DEVICE_CAMERA) && !dev->usb_cam.dev && desc.idProduct == PID_XTION) {
