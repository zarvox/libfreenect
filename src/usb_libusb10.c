...
FN_INTERNAL int fnusb_open_subdevices(freenect_device *dev, int index)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		res = 0;
		// Search for the indexth camera
		if (desc.idProduct == PID_NUI_CAMERA || desc.idProduct == PID_K4W_CAMERA) {
			if (nr_cam == index) {
				// Find the matching audio and maybe motor devices by topology.
				libusb_device* camera = devs[i];
				libusb_device* audio = NULL;
				libusb_device* motor = NULL;
				libusb_device* hub = libusb_get_parent(camera);
				int j;
				struct libusb_device_descriptor desc2;
				for (j = 0; j < cnt; j++) {
					if (hub == libusb_get_parent(devs[j])) { // Find matching parent device.
						libusb_get_device_descriptor(devs[j], &desc2);
						if (desc2.idVendor == VID_MICROSOFT) {
							if (desc2.idProduct == PID_NUI_MOTOR) {
								motor = devs[j];
							} else if (desc2.idProduct == PID_NUI_AUDIO || fnusb_is_pid_k4w_audio(desc2.idProduct)) {
								audio = devs[j];
							}
						}
					}
				...
				// Open the devices that were requested, camera, then motor, then audio
				...
				if (motor != NULL && ctx->enabled_subdevices & FREENECT_DEVICE_MOTOR) {
					libusb_get_device_descriptor(motor, &desc2);
					dev->usb_motor.VID = desc2.idVendor;
					dev->usb_motor.PID = desc2.idProduct;
				...
				if (audio != NULL && ctx->enabled_subdevices & FREENECT_DEVICE_AUDIO) {
					libusb_get_device_descriptor(audio, &desc2);

					dev->usb_audio.VID = desc2.idVendor;
					dev->usb_audio.PID = desc2.idProduct;
					...
					if (num_interfaces >= 2) {
						...
						res = libusb_get_string_descriptor_ascii(dev->usb_audio.dev, desc2.iSerialNumber, string_desc, 256);
						if (res < 0) {
						...
						if (desc2.idProduct == PID_NUI_AUDIO && ctx->fn_fw_nui_ptr && ctx->fn_fw_nui_size > 0) {
							FN_SPEW("loading firmware from memory\n");
							res = upload_firmware_from_memory(&dev->usb_audio, ctx->fn_fw_nui_ptr, ctx->fn_fw_nui_size);
						} else if (desc2.idProduct == PID_K4W_AUDIO && ctx->fn_fw_k4w_ptr && ctx->fn_fw_k4w_size > 0) {
							FN_SPEW("loading firmware from memory\n");
						...
						for (loops = 0; loops < 10; loops++) {
							...
							for (dev_index = 0; dev_index < num_new_devs; ++dev_index) {
								...
								if (new_dev_desc.idVendor == VID_MICROSOFT && (new_dev_desc.idProduct == PID_NUI_AUDIO || fnusb_is_pid_k4w_audio(desc2.idProduct))) {
